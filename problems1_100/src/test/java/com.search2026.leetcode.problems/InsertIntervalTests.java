...
public class InsertIntervalTests {
    ...
    public void testSolution() {
        ...
        List<Interval> res = sol.insert(intervals, new Interval(2, 5));
        ...
        res = sol.insert(intervals, new Interval(4, 9));
        assertEquals(3, res.size());
        assertEquals(1, res.get(0).start);
        assertEquals(2, res.get(0).end);
        assertEquals(3, res.get(1).start);
        assertEquals(10, res.get(1).end);
        assertEquals(12, res.get(2).start);
        assertEquals(16, res.get(2).end);
    }
