    ...
    public class Solution_2 {
        public double findMedianSortedArrays(int nums1[], int nums2[]) {
            ...
            while (left < right) {
                ...
                int midB = mid - midA;
                if (nums1[midA] < nums2[midB]) left = midA + 1;
                else right = midA;
            }
            ...
            int a = Math.max(left > 0 ? nums1[left - 1] : Integer.MIN_VALUE,
                    mid - left >= 0 ? nums2[mid - left] : Integer.MIN_VALUE);
            if (((n + m) & 1) == 1) return (double) a;

            // if (n+m) is even, the median can be calculated by
            //      median = (max(A[left-1], B[mid-l]) + min(A[left], B[mid-left+1]) / 2.0
            // also, there are some corner cases to take care of.
            int b = Math.min(left < n ? nums1[left] : Integer.MAX_VALUE,
                    mid - left + 1 < m ? nums2[mid - left + 1] : Integer.MAX_VALUE);
            return (a + b) / 2.0;
        }
    }
